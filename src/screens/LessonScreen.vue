<template>
  <q-page v-if="unitData">
    ...
    <q-scroll-area
      ...
      :class="{'no-pointer-events' : isFullscreen}"
    >
      <div class="flex column no-wrap">
        ...
        >
          <div class="absolute-bottom text-h6 text-left q-pa-sx">{{unitData.title}}</div>
        </q-img>
        <div
          class="lesson-content-container q-mx-md q-mb-md"
          v-for="(element, index) in unitData.content"
          :key="index"
        >
          <span
          ...
          <span
            ...
            v-html="element.value"
            class="text-body1 lesson-paragraph"
          ></span>
          <q-img
          ...
          <div v-else-if="element.__typename == 'VideoType'" class="lesson-video-wrapper">
            <youtube
              id="video"
              :video-id="getYoutubeId(element.value)"
        ...
        <div
          v-show="isFullscreen"
          style="height: 12rem"
        >Placeholder to keep layout when carousel goes Fullscreen</div>
        ...
        <q-btn
          v-if="!lessonsCompleted.includes(parseInt(unitData.id))"
          style="margin: 2rem 2rem 6rem 2rem"
          color="green"
          @click="markLessonCompleted(unitData.id, true)"
        >Check</q-btn>
        <q-btn
          v-else
          style="margin: 2rem 2rem 6rem 2rem"
          color="white"
          text-color="black"
          @click="markLessonCompleted(unitData.id, false)"
        >uncheck</q-btn>
      </div>
    </q-scroll-area>
  </q-page>
